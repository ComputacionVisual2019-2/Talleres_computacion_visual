//ButtonList library made by Julian Diaz juddiazci@unal.edu.co
import java.util.List;

public class ButtonList implements OnClickListener{
  private float x;
  private float y;
  ...
  public ButtonList addSubmenuButton(float w, float h, String text,float maxHeight, OnClickListener clickListener){
    ...
    Button menuButton = new MenuButton(w,h,text,clickListener,bl,this);
  ...
  public void onClick(int x, int y){
    for(int i=0;i<buttons.size();i++){
      ...
      if(b.isMenuButton() && b.isActive()){
        if(x<b.getWidth()+b.getX() && x>b.getX() && y<b.getHeight()+b.getY() && y>b.getY()){
          b.onClick(x,y);
        }else{
          ((MenuButton) b).childrenClick(x,y);
        }
  ...
  public void setActive(boolean active){
    if(active){
      Events.getInstance().addOnClickListener(this);
    }else{
      Events.getInstance().removeOnClickListener(this);
    }
  }
}
...
public class MenuButton extends Button{
  ...
  public MenuButton(float w, float h, String text, OnClickListener onClick, ButtonList bl, ButtonList parent){
  ...
  public void onClick(int x, int y){
    if(getOnClickListener()!=null){
      getOnClickListener().onClick(x,y);
    }
    parent.clearActiveMenu();
    setIsActive(true);
  }
  
  public void childrenClick(int x, int y){
    bl.onClick(x,y);
  }
